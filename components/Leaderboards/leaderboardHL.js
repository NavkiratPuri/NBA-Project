...
const LeaderboardHL = ({ highScores }) => {
  return (
    <div className="p-4 bg-gray-800 text-white min-h-screen">
      <h1 className="text-3xl font-bold text-center mb-4">Higher-Lower Game</h1>
            ...
            {highScores.map((user, index) => {
              ...
              return (
                <tr key={index} className={`${rowClass} text-white`}>
                  ...
                  <td className="py-2 px-4 border-b border-gray-700 text-center">{user.highScoreHL}</td>
                </tr>
              );
            })}
          </tbody>
